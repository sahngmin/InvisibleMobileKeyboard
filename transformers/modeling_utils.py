...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    r""" Base class for all models.
        ...
        Class attributes (overridden by derived classes):
            ...
            - ``load_tf_weights``: a python ``method`` for loading a TensorFlow checkpoints in a PyTorch model, taking as arguments:

                - ``model``: an instance of the relevant subclass of :class:`~transformers.PreTrainedModel`,
                - ``config``: an instance of the relevant subclass of :class:`~transformers.PretrainedConfig`,
                - ``path``: a path (string) to the TensorFlow checkpoints.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Parameters:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a `tensorflow index checkpoints file` (e.g. `./tf_model/model.ckpt.index`). In this case, ``from_tf`` should be set to True and a configuration object should be provided as ``config`` argument. This loading path is slower than converting the TensorFlow checkpoints in a PyTorch model using the provided conversion scripts and loading the PyTorch model afterwards.
        ...
        Examples::
            ...
            # Loading from a TF checkpoints file instead of a PyTorch model (slower)
        ...
        if pretrained_model_name_or_path is not None:
            if pretrained_model_name_or_path in cls.pretrained_model_archive_map:
                ...
                if from_tf and os.path.isfile(os.path.join(pretrained_model_name_or_path, TF_WEIGHTS_NAME + ".index")):
                    # Load from a TF 1.0 checkpoints
                    archive_file = os.path.join(pretrained_model_name_or_path, TF_WEIGHTS_NAME + ".index")
                elif from_tf and os.path.isfile(os.path.join(pretrained_model_name_or_path, TF2_WEIGHTS_NAME)):
                    # Load from a TF 2.0 checkpoints
                    archive_file = os.path.join(pretrained_model_name_or_path, TF2_WEIGHTS_NAME)
                elif os.path.isfile(os.path.join(pretrained_model_name_or_path, WEIGHTS_NAME)):
                    # Load from a PyTorch checkpoints
                ...
                assert (
                    from_tf
                ), "We found a TensorFlow checkpoints at {}, please set from_tf to True to load from this checkpoints".format(
                ...
                if from_tf:
                    raise EnvironmentError(
                        "Loading a PyTorch model from a TF checkpoints is not supported when using a model identifier name."
                    )
        ...
        if state_dict is None and not from_tf:
            try:
                ...
                raise OSError(
                    "Unable to load weights from pytorch checkpoints file. "
                    "If you tried to load a PyTorch model from a TF 2.0 checkpoints, please set from_tf=True. "
                )
        ...
        if from_tf:
            if resolved_archive_file.endswith(".index"):
                # Load from a TensorFlow 1.X checkpoints - provided by original authors
