    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Params:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a `PyTorch, TF 1.X or TF 2.0 checkpoints file` (e.g. `./tf_model/model.ckpt.index`). In the case of a PyTorch checkpoints, ``from_pt`` should be set to True and a configuration object should be provided as ``config`` argument.

            from_pt: (`Optional`) Boolean
                Set to True if the Checkpoint is a PyTorch checkpoints.
